<div class="sm:grid sm:grid-cols-4 sm:gap-4 sm:items-start sm:pt-5 mb-2 form-field">
    ...
    <div class="mt-1 sm:mt-0 sm:col-span-3">
        <div class="rounded-md shadow-sm">
            <input type="email" name="{{ $field->name }}" id="{{ $field->id }}" value="{{ $field->value }}" class="form-input block w-full transition duration-150 ease-in-out sm:text-base sm:leading-5"{{ $field->required ? ' required' : '' }} />
        </div>
