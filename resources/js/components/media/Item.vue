<template>
    <div class="shadow-sm rounded-md sm:overflow-hidden" :class="[activeClass]" @click.prevent="isActive =! isActive">
        <div class="bg-white p-2" :class="[
            ...
            <div class="bg-gray-300 flex items-center justify-center block overflow-hidden" :class="[
                (isActive ? '' : 'h-40')
            ]">
                <img :src="file.url" class="object-cover min-h-full min-w-full block" v-if="isImage">
            </div>
        </div>
        <div class="bg-white p-3 flex-1 pl-6" v-if="isActive">
            ...
            <div class="mt-4">
                ...
                <a :href="file.url" target="_blank" class="block text-gray-900 text-indigo-600 truncate pr-10">{{ file.url }}</a>
            </div>
            <div class="mt-4">
                <span class="inline-flex rounded-md shadow-sm">
                    <button type="button" class="inline-flex items-center px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-500 focus:outline-none focus:border-indigo-700 focus:shadow-outline-indigo active:bg-indigo-700 transition ease-in-out duration-150" @click.prevent="chooseImage(file.url)">
                        Choose Image
                    </button>
                </span>
            </div>
